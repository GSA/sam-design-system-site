...
ObservableStream.prototype._write = function (chunk, encoding, done) {
  ...
  const componentRegex = new RegExp(
    /(?=@Component)(?:[^\}]*selector[^:]*:[^A-Za-z]*)([A-Za-z-]*)(?:[\s\S]*)(?:export\s+class\s)(\w+)*/g
  );
  const directiveRegex = new RegExp(
    /(?=@Directive)(?:[^\}]*selector[^:]*:[^A-Za-z]*)([A-Za-z-]*)(?:[\s\S]*)(?:export\s+class\s)(\w+)*/g
  );
  const pipeRegex = new RegExp(
    /(?=@Pipe)(?:[^\}]*selector[^:]*:[^A-Za-z]*)([A-Za-z-]*)(?:[\s\S]*)(?:export\s+class\s)(\w+)*/g
  );
...
const uiKitDir = path.resolve(
  __dirname + '/../node_modules/@gsa-sam/sam-ui-elements/src/ui-kit'
);
...
let dirStream = generateObs(uiKitDir);

let docStream = generateObs(docsDir);

dirStream.merge(docStream).subscribe(
  (fileObj) => {
    if (!fileObj) return;
    ...
    generateDirsInPath(fileObj.path, (err) => {
      ...
      let componentFileName = path.resolve(
        docsDir,
        filePath + '/component-example.ts'
      );
      let componentTemplateFileName = path.resolve(
        docsDir,
        filePath + '/component-example.html'
      );
      ...
      createFile(markdownFileName, '# ' + fileObj.component, (err) =>
        console.error(err)
      );
      // Create component file
      createFile(componentFileName, generateComponentStub(fileObj), (err) =>
        console.error(err)
      );
