...
export class BaseExampleComponent implements OnInit {
  ...
  public sections = ['markdown', 'design', 'guidance', 'implementation'];
  ...
  public fetchSection(section: string) {
    const fileName = section === 'markdown' ? 'documentation' : section;

    return this.mdService.get(`${this.base}${fileName}.md`).subscribe(
      (data: any) => {
