...
function generateModuleString(target) {
  ...
  let imports = files.reduce((prev, curr) => {
    return prev.concat(
      `import { ${curr.component} } from './${curr.link}/component-example';\n`
    );
  }, '');
  ...
  let routes = files.reduce((prev, curr) => {
    if (curr.doc2Flag) {
      return prev.concat(
        `\n  { path: '${
          curr.link
        }', component: BaseDocPageComponent, data: {\n    path: '${
          curr.docPath
        }',\n    componentName: '${curr.component.replace(
          'ExampleComponent',
          ''
        )}',
			sections: ['` +
          curr.docSections.join("','") +
          `']}, children: [
